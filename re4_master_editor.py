...
def is_game_running(exe_path):
    """Check if bio4.exe process is currently running — fast, no subprocess."""
    exe_name = os.path.basename(exe_path).lower()
    try:
        import psutil
        return any(p.name().lower() == exe_name for p in psutil.process_iter(["name"]))
    except ImportError:
        pass
    try:
        # fallback: Windows-only WMIC (faster than tasklist)
        import subprocess
        result = subprocess.run(
            ["wmic", "process", "where",
             f"name='{exe_name}'", "get", "name"],
    ...
    def refresh(self, applied, locked, detected):
        ...
        if has_num and is_num_tog:
            ...
            self.toggle_btn.configure(
                ...
                highlightbackground=GREEN if applied else "#888"
            )
        ...
        if locked and applied:
            ...
            self.toggle_btn.configure(
                ...
                highlightbackground="#666", state="normal"  # keep enabled to show message
            )
            self.status_var.set("[L]")
            self.status_lbl.configure(fg="#c85a2a", bg=bg)
            ...
            self.toggle_btn.configure(
                ...
                highlightbackground="#888", state="normal"
            )
            self.status_var.set("")
            self.status_lbl.configure(fg=MUTED, bg=bg)
            self.name_lbl.configure(fg=TEXT_MAIN, bg=bg, font=FONT_NORMAL)
...
def _launch_data_to_udas(root):
    ...
    def get_tools_dir():
        base = os.path.dirname(os.path.abspath(__file__)) if hasattr(_sp, '__file__') else os.getcwd()
        return os.path.join(os.path.dirname(os.path.abspath(__file__)), "RE4 MASTER EDITOR", "Scripts", "Data To Udas")
    def find_exe(name):
        td = get_tools_dir()
...
class RE4MasterEditor(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(left, text="  v1.0.0", fg="#666666", bg="#0c0c0c",
...
if __name__ == "__main__":
    ...
    if is_first:
        def _on_lang_chosen(lang):
            # Restart so the chosen language is fully applied everywhere
            import subprocess
            app.after(150, lambda: (
                subprocess.Popen([__import__("sys").executable] + __import__("sys").argv),
                app.quit(),
                app.destroy()
            ))
